interface CancellationOperatorNotificationParams {
...
export function cancellationOperatorNotificationEmail(params: CancellationOperatorNotificationParams): {
  to: string;
  subject: string;
  html: string;
} {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
  ...
  <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
    ...
    ${customerName ? `<p style="margin: 0 0 4px 0;"><strong>Customer:</strong> ${customerName}</p>` : ""}
    ${customerEmail ? `<p style="margin: 0;"><strong>Email:</strong> ${customerEmail}</p>` : ""}
  </div>

  ${refundIssued
    ? `<p style="margin-bottom: 24px; color: #059669;">A full refund has been processed via PayPal.</p>`
    : `<p style="margin-bottom: 24px; color: #d97706;">No PayPal capture ID was found — you may need to issue a refund manually via the PayPal Dashboard.</p>`
  }
